...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!r.rows.length)
      return res
        .status(401)
        .json({ success: false, message: "Account পাওয়া যায়নি।" });
    ...
    res.json({
      success: true,
      token,
      dev: { id: dev.id, name: dev.name, email: dev.email },
    });
  } catch (err) {
...
router.get("/stats", devAuth, async (req, res) => {
  try {
    const [tenants, admins, logs] = await Promise.all([
      ...
      masterDb.query(
        "SELECT * FROM dev_logs ORDER BY created_at DESC LIMIT 20",
      ),
    ]);
    res.json({
      success: true,
...
router.post("/reset-password", devAuth, async (req, res) => {
  ...
  if (!email || !new_password)
    return res
      .status(400)
      .json({ success: false, message: "Email ও নতুন password দিন।" });
  try {
    ...
    if (!r.rows.length)
      return res
        .status(404)
        .json({ success: false, message: "Admin পাওয়া যায়নি।" });
    ...
    res.json({
      success: true,
      message: `${r.rows[0].name}-এর password reset হয়েছে।`,
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/toggle-admin/:id", devAuth, async (req, res) => {
  try {
    const cur = await masterDb.query("SELECT * FROM super_admins WHERE id=$1", [
      req.params.id,
    ]);
    if (!cur.rows.length)
      return res
    ...
    await masterDb.query("UPDATE super_admins SET is_active=$1 WHERE id=$2", [
      newStatus,
      req.params.id,
    ]);
    ...
    await masterDb.query(
      ...
      [
        req.dev.id,
        "toggle_admin",
        `${cur.rows[0].email}: ${newStatus ? "enabled" : "disabled"}`,
      ],
    );

    res.json({
      success: true,
      message: `${newStatus ? "সক্রিয়" : "নিষ্ক্রিয়"} করা হয়েছে।`,
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.post("/toggle-center/:id", devAuth, async (req, res) => {
  try {
    const cur = await masterDb.query("SELECT * FROM tenants WHERE id=$1", [
      req.params.id,
    ]);
    if (!cur.rows.length)
      return res
    ...
    await masterDb.query("UPDATE tenants SET active=$1 WHERE id=$2", [
      newStatus,
      req.params.id,
    ]);
    ...
    await masterDb.query(
      ...
      [
        req.dev.id,
        "toggle_center",
        `${cur.rows[0].slug}: ${newStatus ? "enabled" : "disabled"}`,
      ],
    );

    res.json({
      success: true,
      message: `Center ${newStatus ? "সক্রিয়" : "নিষ্ক্রিয়"} করা হয়েছে।`,
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
