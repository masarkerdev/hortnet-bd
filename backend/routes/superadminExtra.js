...
router.post(
  "/category-master",
  saAuth,
  requireRole("director", "program_officer"),
  async (req, res) => {
    const { name_bn, name_en, base_group } = req.body;
    if (!name_bn || !base_group) {
      return res
        .status(400)
        .json({ success: false, message: "ক্যাটাগরির নাম ও গ্রুপ দিন।" });
    }
    try {
      const existing = await masterDb.query(
        "SELECT id FROM category_master WHERE name_bn = $1",
        [name_bn],
      );
      let categoryId;
      ...
      for (const [slug, tenant] of Object.entries(tenants)) {
        ...
        try {
          ...
          const localExisting = await db.query(
            "SELECT id FROM categories WHERE name_bn = $1",
            [name_bn],
          );
          if (localExisting.rows.length > 0) {
            await db.query(
              "UPDATE categories SET category_master_id=$1, base_group=$2 WHERE id=$3",
...
router.put(
  ...
  async (req, res) => {
    ...
    if (!name_bn || !base_group) {
      return res
        .status(400)
        .json({ success: false, message: "নাম ও গ্রুপ দিন।" });
    }
    try {
      const old = await masterDb.query(
        "SELECT name_bn FROM category_master WHERE id=$1",
        [id],
      );
      if (!old.rows.length)
        return res
      ...
      res.json({
        success: true,
        message: `"${name_bn}" আপডেট ও ${syncedCount}টি সেন্টারে সিঙ্ক হয়েছে।`,
      });
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    const { id } = req.params;
    try {
      const cat = await masterDb.query(
        "SELECT name_bn FROM category_master WHERE id=$1",
        [id],
      );
      if (!cat.rows.length)
      ...
      await masterDb.query(
        "UPDATE category_master SET is_active=false, updated_at=now() WHERE id=$1",
        [id],
      );

      res.json({
        success: true,
        message: `"${cat.rows[0].name_bn}" নিষ্ক্রিয় করা হয়েছে। সেন্টারের existing চারার তথ্য অক্ষত আছে।`,
      });
    } catch (err) {
      res.status(500).json({ success: false, error: err.message });
    }
...
router.post(
  "/center/:slug/set-category-targets",
  saAuth,
  requireRole("director", "program_officer"),
  async (req, res) => {
    ...
    if (!fy || !Array.isArray(targets) || targets.length === 0) {
      return res.status(400).json({
        success: false,
        message: "অর্থবছর ও লক্ষ্যমাত্রার তালিকা দিন।",
      });
    }
    try {
      const tenants = await getTenants();
      ...
      for (const t of targets) {
        const targetType =
          "category_" + t.mother_category_name_bn.replace(/\s+/g, "_");
...
router.get("/center/:slug/category-targets", saAuth, async (req, res) => {
  ...
  if (!fy)
    return res
      .status(400)
      .json({ success: false, message: "অর্থবছর দিন (fy)।" });
  try {
    const tenants = await getTenants();
    const tenant = tenants[slug];
    if (!tenant)
    ...
    let grandTotal = 0;
    r.rows.forEach((row) => {
      const motherCatName = row.target_type
        .replace("category_", "")
        .replace(/_/g, " ");
    ...
    res.json({
      success: true,
      data: { targets: targetsMap, grand_total: grandTotal },
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/report/topsheet", saAuth, async (req, res) => {
  ...
  if (!fy)
    return res
      .status(400)
      .json({ success: false, message: "অর্থবছর (fy) দিন।" });
  ...
  try {
    const tenants = await getTenants();
    const tenantList =
      scope === "center" && slug ? { [slug]: tenants[slug] } : tenants;
    ...
    const report = Object.values(agg).sort(
...
router.get("/report/category-detail", saAuth, async (req, res) => {
  ...
  if (!mc)
    return res
      .status(400)
      .json({ success: false, message: "সঠিক mother_category দিন।" });
  ...
  try {
    const tenants = await getTenants();
    const tenantList =
      scope === "center" && onlySlug
        ? { [onlySlug]: tenants[onlySlug] }
        : tenants;
    ...
    const data = Object.values(merged).sort(
      (a, b) =>
        ...
        (a.variety || "").localeCompare(b.variety || "", "bn"),
    );
    res.json({
      success: true,
      mother_category,
  ...
  try {
    ...
    for (const [slug, tenant] of Object.entries(tenants)) {
      ...
      try {
        ...
        const [curR, prevR, curIncomeR, prevIncomeR] = await Promise.all([
          db.query(
        ...
        curR.rows.forEach((r) => {
          const label = bucketCategorySA(r.name_bn);
    ...
    const rows = GOV_CATEGORIES_SA.map((g) => {
