...
import {
  IcGrid, IcLeaf, IcClipboard, IcTree, IcBox, IcArchive, IcAlert, IcReceipt,
  IcCoin, IcUsers, IcChart, IcUser, IcRecycle, IcBin, IcSettings, IcMenu, IcLogout, IcSearch, IcPlus, IcCart, IcEye,
} from './icons';
...
const TITLES = {
  '/': 'ড্যাশবোর্ড', '/seedlings': 'চারা তালিকা', '/production': 'উৎপাদন রেজিস্টার',
  '/mother-plants': 'মাদার প্ল্যান্ট', '/batches': 'ব্যাচ ম্যানেজমেন্ট', '/stock': 'স্টক রেজিস্টার',
  '/opening-stock': 'প্রারম্ভিক স্টক', '/damages': 'ক্ষতি / নষ্ট', '/sales': 'বিক্রয় ও চালান',
  '/income': 'অন্যান্য আয়', '/customers': 'গ্রাহক তালিকা', '/reports': 'রিপোর্ট ও বিশ্লেষণ',
  '/users': 'ব্যবহারকারী', '/employees': 'জনবল তালিকা', '/settings': 'সেটিংস', '/recycle-bin': 'Recycle Bin',
};
...
export default function Layout() {
  ...
  const fys = fyOptions();
  const [fy, setFy] = useState(() => Number(localStorage.getItem('hc_fy')) || fys[0]);
  useEffect(() => { if (!localStorage.getItem('hc_fy')) localStorage.setItem('hc_fy', String(fy)); }, []);
  ...
  function changeFy(v) { localStorage.setItem('hc_fy', String(v)); setFy(v); }

  function loadSeedlings() {
    if (!seedlings.length) api.get('/seedlings', { params:{ limit:1000 } }).then((r)=>setSeedlings(r.data?.data||[])).catch(()=>{});
  }
  function openBatch() {
    setBatchOpen(true);
    loadSeedlings();
    if (!mothers.length) api.get('/mother-plants').then((r)=>setMothers(r.data?.data||[])).catch(()=>{});
  }
  function openSale() { setSaleOpen(true); loadSeedlings(); }
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2">
          {SECTIONS.map((sec) => {
            ...
            <div key={sec.en} className="mb-3">
              <div className="px-2 pb-1 text-[10px] font-semibold tracking-wider" style={{ color: 'var(--sa)' }}>
                {sec.bn} / {sec.en}
              </div>
              <div className="space-y-0.5">
                {items.map((it) => (
                  <NavLink
                    key={it.to} to={it.to} end={it.end} onClick={() => setOpen(false)}
                    className="flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition hover:bg-white/5"
                    style={({ isActive }) => isActive
                      ? { background: 'var(--sa)', color: '#fff', fontWeight: 600 }
                      : { color: 'var(--st)' }}
                  >
        ...
        >
          ...
          <span className="leading-tight text-left">
            ...
            <span className="block text-[11px]" style={{ color: 'var(--st)' }}>{user?.role}</span>
          </span>
          <IcLogout className="ml-auto h-[18px] w-[18px]" style={{ color: '#f3b4b4' }} />
        </button>
      </aside>

      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b bg-white px-4 lg:px-6" style={{ borderColor: 'var(--bd)' }}>
          <button onClick={() => setOpen(true)} className="rounded-lg border p-1.5 lg:hidden" style={{ borderColor: 'var(--bd)' }} aria-label="মেনু"><IcMenu className="h-5 w-5" /></button>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {can('sale') && (
              <button onClick={openSale} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-medium text-white" style={{ background: 'var(--g600)' }}>
                <IcCart className="h-4 w-4" /> বিক্রয়
              </button>
            )}
            <div className="hidden items-center gap-1.5 md:flex">
              <span className="text-[12px]" style={{ color: 'var(--tm)' }}>অর্থবছর:</span>
            ...
            <button onClick={() => setProfileOpen(true)} title="আমার প্রোফাইল"><Avatar t={initials} /></button>
          </div>
        </header>
...
function ProfileModal({ open, onClose, user, logout }) {
  const RN = { admin:'Admin', manager:'Manager', production_officer:'Production Officer', sales_operator:'Sales Operator', viewer:'Viewer' };
  ...
  useEffect(() => { if (open) { setName(user?.name||''); setEmail(user?.email||''); setNpw(''); setCon(''); setOtp(''); setOtpSent(false); setMsg(''); setErr(''); } }, [open, user]);

  async function saveInfo() {
    setBusy(true); setErr(''); setMsg('');
    try { const r = await api.put('/auth/update-profile', { name, email }); if (r.data?.success) setMsg('নাম ও ইমেইল আপডেট হয়েছে ✅'); else setErr(r.data?.message||'সমস্যা'); }
    catch (e) { setErr(e?.response?.data?.message||'সমস্যা হয়েছে'); } finally { setBusy(false); }
  }

  async function sendOtp() {
    if (!npw || npw.length < 6) { setErr('নতুন পাসওয়ার্ড কমপক্ষে ৬ অক্ষর'); return; }
    if (npw !== con) { setErr('পাসওয়ার্ড মিলছে না'); return; }
    setBusy(true); setErr(''); setMsg('');
    try { const r = await api.post('/auth/password-otp'); if (r.data?.success) { setOtpSent(true); setMsg(r.data.local ? 'ইমেইলে OTP পাঠানো হয়েছে (লোকাল: 123456)' : 'আপনার ইমেইলে OTP পাঠানো হয়েছে।'); } else setErr(r.data?.message||'OTP পাঠানো যায়নি'); }
    catch (e) { setErr(e?.response?.data?.message||'OTP পাঠাতে সমস্যা'); } finally { setBusy(false); }
  }

  async function confirmChange() {
    if (otp.trim().length !== 6) { setErr('৬ সংখ্যার OTP দিন'); return; }
    setBusy(true); setErr(''); setMsg('');
    try {
      const r = await api.post('/auth/change-password-verified', { otp: otp.trim(), new_password: npw });
      if (r.data?.success) {
        if (r.data.applied) { setMsg('পাসওয়ার্ড পরিবর্তন হয়েছে ✅ পুনরায় Login করুন...'); setTimeout(()=>logout(), 1600); }
        else { setMsg('ইমেইল যাচাই সম্পন্ন ✅ Admin-এর অনুমোদনের পর পাসওয়ার্ড কার্যকর হবে।'); setOtpSent(false); setNpw(''); setCon(''); setOtp(''); }
      } else setErr(r.data?.message||'সমস্যা');
    } catch (e) { setErr(e?.response?.data?.message||'সমস্যা হয়েছে'); } finally { setBusy(false); }
  }

  return (
    <Modal open={open} onClose={onClose} title={`আমার প্রোফাইল — ${RN[user?.role] || user?.role || ''}`}>
      <div className="space-y-3">
        <div className="flex items-center gap-3 rounded-xl p-3" style={{ background:'var(--g50)' }}>
          <div className="flex h-12 w-12 items-center justify-center rounded-full text-[15px] font-semibold text-white" style={{ background:'var(--g600)' }}>{(name||'U').trim().slice(0,2).toUpperCase()}</div>
          <div><div className="font-semibold">{name}</div><div className="text-[12px]" style={{ color:'var(--tm)' }}>{RN[user?.role] || user?.role}</div></div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div><label className="field-label">নাম</label><input className="field-input" value={name} onChange={(e)=>setName(e.target.value)} /></div>
          <div><label className="field-label">ইমেইল</label><input className="field-input" value={email} onChange={(e)=>setEmail(e.target.value)} /></div>
        </div>
        <button onClick={saveInfo} disabled={busy} className="rounded-lg border px-3 py-2 text-[13px] font-medium" style={{ borderColor:'var(--bd)' }}>নাম/ইমেইল সংরক্ষণ</button>

        <div className="rounded-xl border p-3" style={{ borderColor:'var(--bd)' }}>
          <div className="mb-1 text-[13px] font-semibold">পাসওয়ার্ড পরিবর্তন</div>
          <div className="mb-2 text-[11px]" style={{ color:'var(--tm)' }}>
            {isAdmin ? 'ইমেইল যাচাই (OTP) করলেই সরাসরি পরিবর্তন হবে।' : 'ইমেইল যাচাই (OTP)-এর পর Admin-এর অনুমোদন লাগবে।'}
          </div>
          {!otpSent ? (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="field-label">নতুন পাসওয়ার্ড</label>
                  <div className="relative"><input type={showN?'text':'password'} className="field-input pr-10" value={npw} onChange={(e)=>setNpw(e.target.value)} placeholder="কমপক্ষে ৬ অক্ষর" />
                  <button type="button" onClick={()=>setShowN(!showN)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color:'var(--tm)' }}><IcEye className="h-4 w-4" /></button></div>
                </div>
                <div><label className="field-label">নিশ্চিত করুন</label><input type={showN?'text':'password'} className="field-input" value={con} onChange={(e)=>setCon(e.target.value)} placeholder="আবার লিখুন" /></div>
              </div>
              <button onClick={sendOtp} disabled={busy} className="btn-primary mt-2.5 w-full">{busy?'...':'📧 ইমেইল যাচাই করুন (OTP)'}</button>
            </>
          ) : (
            <>
              <label className="field-label">ইমেইলে আসা ৬ সংখ্যার OTP</label>
              <input className="field-input text-center text-lg tracking-[6px]" maxLength={6} value={otp} onChange={(e)=>setOtp(e.target.value.replace(/\D/g,''))} placeholder="------" />
              <div className="mt-2.5 flex gap-2">
                <button onClick={()=>{setOtpSent(false);setOtp('');setMsg('');}} className="rounded-lg border px-4 py-2.5 text-[13px]" style={{ borderColor:'var(--bd)' }}>← পিছনে</button>
                <button onClick={confirmChange} disabled={busy} className="btn-primary flex-1">{busy?'যাচাই হচ্ছে…':'✓ নিশ্চিত করুন'}</button>
              </div>
            </>
          )}
        </div>

        {msg && <div className="text-[13px]" style={{ color:'var(--g600)' }}>{msg}</div>}
        {err && <div className="text-[13px]" style={{ color:'var(--r600)' }}>{err}</div>}
...
function Avatar({ t }) {
  return <div className="flex h-9 w-9 items-center justify-center rounded-full text-[13px] font-semibold text-white" style={{ background: 'var(--g600)' }}>{t}</div>;
}
function Leaf({ className }) {
  return (<svg className={className} viewBox="0 0 32 32" aria-hidden="true"><path d="M16 28C9 28 5 22 5 15 5 9 9 5 16 4c7 1 11 5 11 11 0 7-4 13-11 13Z" fill="#4f9c68" /><path d="M16 24V9M16 15l5-4M16 19l-5-4" stroke="#15402b" strokeWidth="1.6" strokeLinecap="round" fill="none" /></svg>);
}
