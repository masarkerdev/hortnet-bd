...
export default function MotherPlants() {
  ...
  function load() {
    setLoading(true);
    api.get('/mother-plants').then((r) => setRows(r.data?.data || [])).catch(() => {}).finally(() => setLoading(false));
  }
  useEffect(() => { load(); }, []);
  ...
  async function save() {
    if (!form.variety || !form.location) { setMsg('জাত ও অবস্থান দিন'); return; }
  ...
  async function del(m) {
    if (!(await confirm({ title: `${m.mp_code} (${m.variety}) ডিলেট করবেন?` }))) return;
    try { await api.delete('/mother-plants/' + m.id); load(); }
    catch (e) { alert(e?.response?.data?.message || 'ডিলেট করতে সমস্যা'); }
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center">
        <button onClick={openNew} className="btn-primary ml-auto"><IcPlus className="h-4 w-4" /> নতুন মাদার প্ল্যান্ট</button>
      </div>
            ...
            {loading ? <tr><td colSpan={8} className="lt">লোড হচ্ছে…</td></tr> :
             ...
             rows.map((m) => (
               <tr key={m.id}>
                 ...
                 <td><span className="b bg">সক্রিয়</span></td>
                 <td><button className="act-btn act-del" onClick={()=>del(m)} title="ডিলেট"><IcTrash className="h-[15px] w-[15px]" /></button></td>
               </tr>
             ))}
          </tbody>
        </table>
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title="নতুন মাদার প্ল্যান্ট">
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="চারা (ঐচ্ছিক)">
              <select className="field-input" value={form.seedling_id} onChange={(e)=>setForm({...form,seedling_id:e.target.value})}>
          ...
          <Field label="স্বাস্থ্য">
            <select className="field-input" value={form.health_status} onChange={(e)=>setForm({...form,health_status:e.target.value})}>
              <option value="excellent">চমৎকার</option><option value="good">ভালো</option><option value="weak">দুর্বল</option>
            </select>
          </Field>
          <Field label="মন্তব্য"><textarea rows={2} className="field-input" value={form.notes} onChange={(e)=>setForm({...form,notes:e.target.value})} /></Field>
