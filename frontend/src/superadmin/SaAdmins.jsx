...
export default function SaAdmins() {
  ...
  async function load() {
    try {
      const [ar, cr, cur] = await Promise.all([
        saApi.get("/admins"),
        saApi.get("/tenants"),
        saApi.get("/center-users"),
      ]);
  ...
  function openAssign(a) {
    setAssignId(a.id);
    setAssigned(a.assigned_centers || []);
    setModal("assign");
  }
  ...
  async function save() {
    ...
    if (!editId && !form.password) {
      setMsg("নতুন admin-এর জন্য পাসওয়ার্ড দিন।");
      return;
    }
    setSaving(true);
    setMsg("");
    try {
      ...
      const r = editId
        ? await saApi.put(`/admins/${editId}`, body)
        : await saApi.post("/admins", body);
      if (r.data?.success) {
        setModal(null);
        load();
  ...
  async function doDelete(a) {
    if (
      ...
      return;
    try {
      const r = await saApi.delete(`/admins/${a.id}`);
  ...
  async function saveAssign() {
    setSaving(true);
    try {
      const r = await saApi.put(`/admins/${assignId}/assignments`, {
  ...
  function openCuEdit(u) {
    setCuForm({
      center_slug: u.center_slug,
      name: u.name,
      email: u.email,
      password: "",
      role: u.role,
  ...
  async function saveCu() {
    if (!cuForm.center_slug || !cuForm.name || !cuForm.email || !cuForm.role) {
      setCuMsg("সব তথ্য দিন।");
      return;
    }
    if (!cuEdit && !cuForm.password) {
      setCuMsg("Password দিন।");
      return;
    }
    setCuSaving(true);
    setCuMsg("");
    try {
      ...
      if (cuEdit) {
        r = await saApi.put(
          `/center-users/${cuEdit.center_slug}/${cuEdit.id}`,
          cuForm,
        );
      } else {
      ...
      if (r.data?.success) {
        setCuModal(false);
        load();
      } else setCuMsg(r.data?.message || "সমস্যা");
    } catch (e) {
      setCuMsg(e?.response?.data?.message || "সমস্যা");
    } finally {
      setCuSaving(false);
    }
  }
  ...
  async function toggleCu(u) {
    try {
      const r = await saApi.post(
        `/center-users/${u.center_slug}/${u.id}/toggle`,
      );
      if (r.data?.success) load();
    } catch {}
  }
  ...
  async function resetPassword() {
    ...
    try {
      const r = await saApi.post(
        `/center-users/${resetModal.center_slug}/${resetModal.id}/reset-password`,
    ...
    <div style={{ fontFamily: FONT }}>
      ...
      {activeTab === "sa" && (
        <>
          ...
          {admins.length === 0 ? (
            ...
            >
              <div style={{ fontSize: 40, marginBottom: 12 }}>👥</div>কোনো Admin
              নেই।
            </div>
          ) : (
          ...
          <Modal
            ...
            title={editId ? "Admin সম্পাদনা" : "নতুন Admin তৈরি"}
          ...
          >
            ...
            <Field label="ইমেইল*">
              <Input
                ...
                placeholder="dd.feni@horticulture.bd"
              />
            </Field>
            <Field
              label={`পাসওয়ার্ড${editId ? " (খালি রাখলে পরিবর্তন হবে না)" : "*"}`}
            >
              <Input
                type="password"
                value={form.password}
          ...
          <Modal
            open={modal === "assign"}
            title="Center Assign করুন"
            onClose={() => setModal(null)}
          >
            <p style={{ fontSize: 13, color: C.muted, marginBottom: 14 }}>
              যে center গুলো এই admin দেখবেন সেগুলো select করুন
            </p>
            <div
              style={{
                display: "flex",
              ...
              {centers.map((c) => (
                <label
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    cursor: "pointer",
                    border: `1px solid ${assigned.includes(c.slug) ? "#3b6d11" : C.border}`,
                  }}
                >
                  <input
                    type="checkbox"
                    checked={assigned.includes(c.slug)}
                    onChange={(e) =>
                      setAssigned(
                        e.target.checked
                          ? [...assigned, c.slug]
                          : assigned.filter((s) => s !== c.slug),
                      )
                    }
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted }}>
                      {c.slug} • {c.category} Category • {c.district || ""}
                    </div>
                  </div>
                </label>
              ))}
            ...
            {msg && (
              <div style={{ color: C.red, fontSize: 13, margin: "8px 0" }}>
                {msg}
              </div>
            )}
            <div
      ...
      {activeTab === "center" && (
        <div>
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          padding: "2px 8px",
                          borderRadius: 6,
                          background: u.is_active ? "#e8f5ed" : "#fee2e2",
                          color: u.is_active ? "#1a6b3a" : "#dc2626",
                          fontWeight: 600,
                        }}
                      >
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        fontFamily: FONT,
                        background: u.is_active ? "#fee2e2" : "#e8f5ed",
                        border: `1px solid ${u.is_active ? "#fca5a5" : "#86efac"}`,
                        color: u.is_active ? "#dc2626" : "#1a6b3a",
                      }}
                    >
          ...
          <Modal
            open={cuModal}
            title={cuEdit ? "Center User এডিট" : "নতুন Center User তৈরি"}
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                ...
                <select
                  value={cuForm.center_slug}
                  onChange={(e) =>
                    setCuForm({ ...cuForm, center_slug: e.target.value })
                  }
                  disabled={!!cuEdit}
                  style={{
                    width: "100%",
              ...
              {[
                ["name", "নাম*", "text", "অফিসারের নাম"],
                ["email", "Email*", "email", "officer@dae.gov.bd"],
                [
                  "password",
                  cuEdit
                    ? "নতুন Password (খালি রাখলে পরিবর্তন হবে না)"
                    : "Password*",
                  "password",
                  "••••••••",
                ],
                ...
                <div key={f}>
                  ...
                  <input
                    ...
                    value={cuForm[f]}
                    onChange={(e) =>
                      setCuForm({ ...cuForm, [f]: e.target.value })
                    }
                    placeholder={ph}
                    style={{
                      width: "100%",
              ...
              <div>
                ...
                <select
                  value={cuForm.role}
                  onChange={(e) =>
                    setCuForm({ ...cuForm, role: e.target.value })
                  }
                  style={{
                    width: "100%",
                    padding: "10px 12px",
              ...
              {cuMsg && (
                <div style={{ color: C.red, fontSize: 13 }}>{cuMsg}</div>
              )}
              <div
                style={{
                  display: "flex",
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <p style={{ fontSize: 13, color: C.muted }}>
                <strong>{resetModal?.name}</strong> ({resetModal?.center_name}
                )-এর নতুন password:
              </p>
              <input
                type="text"
