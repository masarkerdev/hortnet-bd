...
function TLBadge({ tl }) {
  const bg =
    { green: "#064e3b", yellow: "#451a03", red: "#450a0a" }[tl] || "#1e293b";
  const color =
    { green: "#4ade80", yellow: "#fbbf24", red: "#f87171" }[tl] || "#94a3b8";
    ...
    <div style={{ position: "relative", width: 52, height: 52, flexShrink: 0 }}>
      <svg width="52" height="52" viewBox="0 0 60 60">
        ...
        <circle
          ...
          stroke={c}
          strokeWidth="6"
          strokeDasharray={`${pct.toFixed(1)} 157`}
          strokeLinecap="round"
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 700, color: c, lineHeight: 1 }}>
          {toBn(score)}
        </div>
  ...
  return (
    <div>
          ...
          <tbody>
            {sorted.map((c, i) => (
              ...
              >
                ...
                <td
                  style={{ padding: "12px 14px", color: V.muted, fontSize: 13 }}
                >
                  {c.district || "—"}
                </td>
                <td style={{ padding: "12px 14px" }}>
                ...
                <td
                  style={{
                    padding: "12px 14px",
                    fontWeight: 700,
                    color: type === "sales" ? V.green : V.text,
                  }}
                >
                ...
                <td
                  style={{
                    padding: "12px 14px",
                    fontWeight: 700,
                    color: type === "production" ? V.purple : V.text,
                  }}
                >
                  {fmtN(c.total_produced)}
                </td>
                <td
                  style={{
                    padding: "12px 14px",
                    fontWeight: 700,
                    color: type === "stock" ? V.amber : V.text,
                  }}
                >
                  {fmtN(c.total_stock)}
                </td>
                <td style={{ padding: "12px 14px" }}>
                  <TLBadge tl={c.traffic_light} />
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {sorted.length >= 2 && (
        ...
        >
          <span>
            🟢 <b style={{ color: "#4ade80" }}>{best?.name_bn}</b> — সর্বোচ্চ
          </span>
          <span>
            🔴 <b style={{ color: "#f87171" }}>{worst?.name_bn}</b> — সর্বনিম্ন
          </span>
        </div>
      )}
    </div>
...
export default function SaCompare() {
  ...
  useEffect(() => {
    saApi
      .get("/stats-all")
      .then((r) => setAllStats(r.data?.data || []))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  const ok = useMemo(
    () => allStats.filter((c) => c.status === "ok"),
    [allStats],
  );
  const districts = useMemo(
    () => [...new Set(ok.map((c) => c.district).filter(Boolean))].sort(),
    [ok],
  );
  const divisions = useMemo(
    () => [...new Set(ok.map((c) => c.division).filter(Boolean))].sort(),
    [ok],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (search)
      d = d.filter((c) =>
        [c.name_bn, c.name_en, c.district, c.division].some((v) =>
  ...
  const ranked = useMemo(
    () => [...filtered].sort((a, b) => b.perf_score - a.perf_score),
    [filtered],
  );

  const sorted = useMemo(() => {
    if (sortType === "production")
      return [...filtered].sort((a, b) => b.total_produced - a.total_produced);
    if (sortType === "stock")
  ...
  function exportPDF() {
    ...
    const rows = filtered
      .map(
        (c, i) =>
          `<tr><td>${i + 1}</td><td><b>${c.name_bn}</b></td><td>${c.category}</td><td>${c.district || "—"}</td><td>৳${c.total_revenue.toLocaleString()}</td><td>${c.total_produced.toLocaleString()}</td><td>${c.total_stock.toLocaleString()}</td><td><b>${c.perf_score}</b>/১০০</td><td>${tlLabel(c.traffic_light)}</td></tr>`,
      )
      .join("");
  ...
  return (
    <div style={{ fontFamily: FONT }}>
      ...
      >
        ...
        >
          <span style={{ fontSize: 13, color: V.muted, fontWeight: 600 }}>
            ফিল্টার:
          </span>
          <select
            value={catF}
          ...
          <select
            value={distF}
            onChange={(e) => setDistF(e.target.value)}
            style={{
              background: V.bg,
              border: `1px solid ${V.border}`,
              color: V.text,
          ...
          <select
            value={divF}
            onChange={(e) => setDivF(e.target.value)}
            style={{
              background: V.bg,
              border: `1px solid ${V.border}`,
              color: V.text,
          ...
          <span style={{ fontSize: 12, color: V.muted, marginLeft: "auto" }}>
            {toBn(filtered.length)}টি center
          </span>
          <button
            onClick={exportCSV}
            style={{
  ...
  @media(max-width:768px){
    .sa-leaderboard-grid{grid-template-columns:1fr !important}
    .sa-lb-card{padding:10px !important; gap:8px !important}
    .sa-lb-info{min-width:70px !important; max-width:100px !important}
    .sa-lb-stats{
    ...
    .sa-lb-stats::-webkit-scrollbar{display:none}
    .sa-lb-stats > div{flex-shrink:0; min-width:44px}
  }
  @media(min-width:769px) and (max-width:1024px){
    .sa-leaderboard-grid{grid-template-columns:repeat(2,1fr) !important}
  }
`}</style>
          <div
            className="sa-leaderboard-grid"
            style={{
              display: "grid",
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, color: V.muted, marginTop: 2 }}>
                    {c.district || ""} • <CatBadge cat={c.category} />
                  </div>
                </div>
                <div
                  className="sa-lb-stats"
                  style={{ display: "flex", gap: 14, flexShrink: 0 }}
                >
                  <div style={{ textAlign: "center" }}>
                    <div
                      style={{ fontSize: 14, color: V.green, fontWeight: 600 }}
                    >
                      ৳{fmtK(c.total_revenue)}
                    </div>
                  ...
                  <div style={{ textAlign: "center" }}>
                    <div
                      style={{
                        fontSize: 14,
                        color: (c.growth_rate || 0) >= 0 ? V.green : V.red,
                        fontWeight: 600,
                      }}
                    >
                      {(c.growth_rate || 0) >= 0 ? "▲" : "▼"}
                      {toBn(Math.abs(c.growth_rate || 0).toFixed(1))}%
                    </div>
                    <div style={{ fontSize: 12, color: V.muted, marginTop: 1 }}>
                      প্রবৃদ্ধি
                    </div>
                  ...
                  <div style={{ textAlign: "center" }}>
                    <div
                      style={{
                        ...
                        borderRadius: "50%",
                        background:
                          { green: V.green, yellow: V.amber, red: V.red }[
                            c.traffic_light
                          ] || V.amber,
                        margin: "0 auto 2px",
                      }}
                    ...
                    >
                      ...
                      >
                        {c.annual_prod_target > 0 ? (
                          `${fmtN(c.annual_prod_target)}টি`
                        ) : (
                          <span style={{ color: V.muted, fontStyle: "italic" }}>
                            নির্ধারিত নেই
                          </span>
                        )}
                      </td>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: FONT,
                  border: `1px solid ${V.border}`,
                  background: sortType === t ? V.green : V.card,
                  color: sortType === t ? "#fff" : V.muted,
                }}
              >
                {l}
              </button>
